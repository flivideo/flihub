...
**Rule: tests must match current implementation behaviour — never change production code to make a test pass.**
Read the actual implementation, observe what it returns, write the test to match that. If `validateChapter('00')` returns `true` today, the test asserts `true`. Correctness of the logic is a separate concern for the PO — not in scope for this campaign.
...
**2026-03-16 — Campaign complete (all 15 items, 298 tests)**

- `projectState.ts` uses **`fs-extra`**, not `fs/promises` — mock `fs-extra` with `{ pathExists, readFile, writeFile }`
- Prune logic fires when **all 4 flags** (safe, parked, stage, annotation) are falsy — not just one
...
- `formatChapterTitle('HELLO-WORLD')` → `''` — all-uppercase is treated as tags, nothing left
...
- `poem-wui.ts` integration test needs **two mocks**: `vi.mock('fs/promises')` + `vi.mock('fs-extra')` for readDirSafe
