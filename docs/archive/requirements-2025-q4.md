...
4. **Expected:** 01-3 still shows with 👻 indicator (shadow-only)
...
│  Chapter 01: Intro                                    │
├──────────────────────────────────────────────────────┤
│  📹 01-1-intro.mov              2:34    245 MB       │  ← Real file
│  👻 01-2-hook.mp4               1:45     12 MB       │  ← Shadow (240p)
│  📹 01-3-overview.mov           3:12    310 MB       │  ← Real file
└──────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────────┐
│  Shadow Videos (Preview Quality)                                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Generate 240p preview videos for collaborators who don't have   │
│  the original recordings.                                        │
│                                                                  │
│  Current project: 45 recordings, 12 shadows (33 missing)         │
│  Estimated size: ~400 MB for missing shadows                     │
│                                                                  │
│  [Generate Shadows]    [Generate All Projects]                   │
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │  ████████████░░░░░░░░░░░░  12/33 files  (36%)           │    │
│  │  Currently processing: 01-15-demo-walkthrough.mov       │    │
│  └─────────────────────────────────────────────────────────┘    │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────────┐
│  Watch Directory (Ecamm Live)                                    │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  🔴 Not configured                                               │
│  Incoming recordings from Ecamm Live will not be detected.       │
│                                                                  │
│  Path: [______________________________________] [Browse]         │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
| State | Display |
|-------|---------|
| Configured + exists | 🟢 Watching: `/path/to/ecamm/` |
| Configured + missing | 🟡 Path not found: `/path/to/ecamm/` |
| Not configured | 🔴 Not configured - Ecamm files will not be detected |
...
```
recordings/-safe/01-1-intro.mov              ← Real safe file
recordings-shadow/-safe/01-1-intro.mp4       ← Shadow safe file
```
...
4. **Understanding Shadow Files**
   ...
   - How to recognize shadow-only content (👻)

5. **What Works Without Video Files**
   - Project list ✅
   - Transcripts (if txt/srt exist) ✅
   - Assets/images ✅
   - Inbox ✅
   - Video playback ❌
   - Transcription ❌
...
```
Has content:  📥 🖼 🎬
Mixed:        📥    🎬    ← middle is blank, not faded
Empty:                    ← all blank
```
...
- Projects may temporarily flip from "editing" → "recording" stage until re-transcribed
...
├── 01-1-intro.txt      # Plain text (existing)
├── 01-1-intro.srt      # Timed subtitles (NEW)
├── 02-1-demo.txt
├── 02-1-demo.srt
```
...
│  ━━━━━━━━━●━━━━━━━━━━━━━━━━━━━━━━━━━ 0:15 / 1:30                │
├─────────────────────────────────────────────────────────────────┤
│  So today we're going to look at the ███BMAD method███ and how  │
│  it can help you build better AI workflows. Let's start with    │
│  the core concepts and then move into practical examples...     │
└─────────────────────────────────────────────────────────────────┘
```
...
┌─────────────────────────────────────────────────────────────────┐
│  ── Segment 1: intro ──                                         │
│  So today we're going to look at the ███BMAD method███...       │
│                                                                  │
│  ── Segment 2: intro ──                                         │
...
Add missing **"scenario"** template — a common early-chapter recording type that's currently not available.
...
│  Mockups               🎨  │
│  ──────────────────────── │
│  API Help              💡  │  ← Future: Claude Code tips panel
└────────────────────────────┘
```

- Click gear → dropdown appears
...
```
Current:  b86-claudemas-01-jump ▼  [📋] [>_]

Proposed: b86-claudemas-01-jump ▼  [···]
                                     ↓
                           ┌────────────────────────┐
                           │  Copy for calendar  📋 │
                           │  Copy full path     >_ │
...
- Single `[···]` or `[▾]` button replaces multiple icons
...
```
┌──────────────────────────────────────────────────────────────────────────────┐
│  FliHub › b86-claudemas-01-jump ▼ [···]                                 [⚙]  │
├──────────────────────────────────────────────────────────────────────────────┤
│  [Incoming] [Recordings] [Transcripts] [Inbox] [Assets] [Thumbs] [Projects]  │
└──────────────────────────────────────────────────────────────────────────────┘
```
...
```
Speed: [1x] [1.5x] [2x●] [2.5x] [3x] [4x]     Size: [N●] [L] [XL]
```
...
├──────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  So this is where we're going to start building out the application.        │
│  The first thing we need to do is set up the project structure.             │
│  And you can see here that we have the main components folder...            │
│                                                                              │
│  [continuing transcript text in readable, well-spaced format]               │
│                                                                              │
└──────────────────────────────────────────────────────────────────────────────┘
```
...
```
┌──────────────────────────────────────────────────────────────────────────────┐
│  📝 Transcript: Chapter 01 - Intro                           [Collapse ▼]    │
├──────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  ── 01-1-intro ──────────────────────────────────────────────────────────── │
│                                                                              │
│  Welcome to this video. Today we're going to look at...                      │
│                                                                              │
│  ── 01-2-intro ──────────────────────────────────────────────────────────── │
│                                                                              │
...
```
┌──────────────────────────────────────────────────────────────────────────────┐
│  [Incoming] [Recordings] [Watch] [Transcripts] [Inbox] [Assets] ...          │
├──────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│              ┌────────────────────────────────────┐                          │
│              │                                    │                          │
...
│              │  start building out the app. The   │                          │
...
**Brand derivation:** Extract from folder name: `v-appydave` → `appydave`
...
```
project/
├── inbox/                  # Raw/unlabeled incoming content
│   ├── raw/
│   ├── dataset/
│   └── presentation/
└── assets/
    ├── images/             # Images + prompts (.txt with same naming)
    ├── thumbs/             # YouTube thumbnails
    └── resources/          # Labeled/finalized resources (Phase 2)
```
...
**Tab order:** Recordings → Inbox → Assets → Thumbnails

**Layout:** Grouped file list with separators (same pattern as chapter rows in Recordings).

```
┌─────────────────────────────────────────────────────────────────┐
│  FliHub > b75-project-name                                      │
├─────────────────────────────────────────────────────────────────┤
│  [Recordings]  [Inbox]  [Assets]  [Thumbnails]                  │
├─────────────────────────────────────────────────────────────────┤
│  Inbox (15 files)                                    [Open 📂]  │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
...
│  raw (3 files · 224 KB)                                          │
│  ─────────────────────────────────────────────────────────────  │
│      screenshot-01.png              124 KB           Dec 14     │
│      screenshot-02.png              98 KB            Dec 14     │
│      notes.txt                      2 KB             Dec 13     │
│                                                                  │
│  ─────────────────────────────────────────────────────────────  │
│  dataset (empty)                                                 │
│  ─────────────────────────────────────────────────────────────  │
│                                                                  │
│  ─────────────────────────────────────────────────────────────  │
│  presentation (12 files · 8.2 MB)                                │
│  ─────────────────────────────────────────────────────────────  │
│      index.html                     45 KB            Dec 12     │
│      styles.css                     12 KB            Dec 12     │
│      assets/logo.svg                8 KB             Dec 12     │
│      slides/01-intro.html           124 KB           Dec 12     │
│      slides/02-overview.html        98 KB            Dec 12     │
│      ...                                                         │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────────────────────────────┐
│  Recording Namer > b71-bmad-poem                                                    │
├─────────────────────────────────────────────────────────────────────────────────────┤
│                                                                                     │
│  ┌─────────────────────────────────────────────────┐  ┌───────────────────────────┐│
│  │                                                 │  │  CHAPTERS (24)            ││
│  │  Project Recordings                             │  ├───────────────────────────┤│
│  │  118 files | 1:18:56        [📄 Transcript]     │  │                           ││
│  │                                                 │  │  0:00  Introduction       ││
│  │  ─────────────────────────────────────────────  │  │  2:15  Setting up...      ││
│  │  Chapter 1: Intro  (4 files · 52s)              │  │  5:42  First demo         ││
│  │  ─────────────────────────────────────────────  │  │  8:30  Scenario           ││
│  │    01-1-intro.mov         11s   13.5 MB         │  │  12:15 Poem Planning  ◀   ││
│  │    01-2-intro.mov         11s   13.0 MB         │  │  18:42 Configuration      ││
│  │    ...                                          │  │  ...                      ││
│  │                                                 │  │                           ││
│  │  (scrollable)                                   │  │           [📋 Copy]       ││
│  │                                                 │  └───────────────────────────┘│
...
- [ ] Copy button → YouTube-format chapter list in clipboard
...
┌─────────────────────────────────────────────────────────────────┐
│  Project Recordings                                              │
│  118 files (118 active, 0 safe) | 1:18:56 | 24 chapters         │
│                                                                  │
│  [📄 Transcript]                                                 │
│       ↑                                                          │
│  Click to open modal                                             │
...
│  ─────────────────────────                                       │
│  So in this video we're going to look at how to set up the      │
│  recording namer application. First thing you need to do is     │
│  make sure you have Node.js installed on your computer...       │
│                                                                  │
│  Chapter 2: Setting up the Environment                          │
│  ─────────────────────────────────────                           │
│  Let's start by cloning the repository from GitHub. Open your   │
│  terminal and navigate to your projects directory...            │
│                                                                  │
│  (scrollable content)                                            │
│                                                                  │
├─────────────────────────────────────────────────────────────────┤
...
- [ ] `[📄 Transcript]` button in Recordings page header
...
**Expected improvement:** 5+ seconds → ~100-200ms
...
- [ ] Clear button (×) next to custom tag input
...
```
📂 FliHub Projects                                              2025-12-07 14:30

PROJECT                              STAGE    CH   FILES    📄     🎬
────────────────────────────────────────────────────────────────────────────────
📌 b72-flivideo-demo                 EDIT     12      28   100%    ✅
📌 b74-opus-multi-agent              REC       4       8     0%    ❌
   b67-vam-s3-staging                -         0       0      -    ❌
   b71-context-engineering           EDIT      9      19   100%    ✅
...
FINAL MEDIA
─────────────────────────────
Video:   ✅ b72-final-v2.mp4 (4.1 GB)
SRT:     ✅ b72-final-v2.srt
...
```
📂 Recordings: b72-flivideo-demo

RECORDING                                        SIZE      DURATION   📄
────────────────────────────────────────────────────────────────────────────────
── Chapter 01: intro ──────────────────────────────────────────────────────────
   01-1-intro.mov                              145.3 MB      2:34    ✅
   01-2-intro-CTA.mov                           89.7 MB      1:23    ✅
── Chapter 02: setup ──────────────────────────────────────────────────────────
   02-1-setup.mov                              234.1 MB      4:12    ✅
   02-2-setup.mov                              178.5 MB      3:01    ⚠️ missing
...
// → "1.5 GB", "234.2 MB", "45.8 KB"

// Relative time
formatAge(date: Date): string
// → "just now", "3h ago", "2d ago", "1w ago"
...
- Note: The blue row highlight already shows the currently selected/active project, so 🔥 is not needed
...
```
┌──────────────────────────────────────────────────────────────────────────────┐
│     │ Project Code              │ Stage │ Ch │ Files │  📄  │ ⓘ            │
├──────────────────────────────────────────────────────────────────────────────┤
│  📌 │ b72-flivideo-demo         │ EDIT  │ 12 │    28 │ 100% │ ⓘ            │
│  📌 │ b74-opus-multi-agent      │  REC  │  4 │     8 │   0% │ ⓘ            │
│     │ b67-vam-s3-staging        │   -   │  0 │     0 │   -  │ ⓘ            │
│     │ b71-context-engineering   │ EDIT  │  9 │    19 │ 100% │ ⓘ            │
│     │ b75-vibe-code-whisper     │  REC  │  7 │    15 │  80% │ ⓘ            │
└──────────────────────────────────────────────────────────────────────────────┘
```
...
1. 📌 pinned projects first (by project code ascending: b67, b68...)
...
```
┌────────────────────────────────────────────────────────────────────────────────────┐
│  AppyDave Projects (13 projects)                                      [+ New]      │
├────────────────────────────────────────────────────────────────────────────────────┤
│     │ Project Code                    │ Stage │ Ch │ Files │  📄   │              │
├────────────────────────────────────────────────────────────────────────────────────┤
│  📌 │   b72-flivideo-recording-demo   │ EDIT  │ 12 │    28 │ 100%  │ ⓘ           │
│  📌 │   b74-multi-agent-workflows     │  REC  │  4 │     8 │   0%  │ ⓘ           │
│     │   b67-vam-s3-staging            │   -   │  0 │     0 │   -   │ ⓘ           │
│     │   b68-prompt-engineering        │  REC  │  2 │     3 │   0%  │ ⓘ           │
│     │   b69-skool-community           │  REC  │  3 │     5 │  40%  │ ⓘ           │
│     │   b70-bmad-method-intro         │ EDIT  │  6 │    14 │ 100%  │ ⓘ           │
│     │   b71-context-engineering       │ EDIT  │  9 │    19 │ 100%  │ ⓘ           │
│     │ ▸ b75-vibe-code-whisper-opus    │  REC  │  7 │    15 │  80%  │ ⓘ    ← blue │
│     │   ...                           │       │    │       │       │              │
├────────────────────────────────────────────────────────────────────────────────────┤
│  + Add new project...                                                              │
└────────────────────────────────────────────────────────────────────────────────────┘
...
│  INCOMING IMAGES (~/Downloads)              [S] [M] [L] [XL]    │
│                                                                  │
│  [Small view - many thumbnails fit]                             │
│  ┌────┐ ┌────┐ ┌────┐ ┌────┐ ┌────┐ ┌────┐ ┌────┐ ┌────┐      │
│  │    │ │    │ │    │ │    │ │    │ │    │ │    │ │    │      │
│  └────┘ └────┘ └────┘ └────┘ └────┘ └────┘ └────┘ └────┘      │
│                                                                  │
│  [Large view - fewer thumbnails, more detail]                   │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐           │
│  │          │ │          │ │          │ │          │           │
│  │          │ │          │ │          │ │          │           │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘           │
...
- Commas in input become dashes (e.g., "sponsor,intro" → "SPONSOR-INTRO")
...
**Status:** ✅ v2 Implemented 2025-11-29
...
|----------|----------|------|-----------|
| Baseline + junk only | 🟢 Green | ⬜ White | - |
| Baseline + better take | 🟡 Yellow | ⬜ White | 🟢 Green |
| Single file | 🟢 Green | - | - |
| Only junk files | Most recent = 🟢 | ⬜ White | - |
...
**Status:** ⚠️ NEEDS CLARIFICATION - Global vs Suggested tags
...
- Chapter separator: thin horizontal line with label in square brackets (e.g., `——[ 01 intro ]——`)
...
└── recordings/
    ├── -safe/           <- completed chapters (inside recordings)
    ├── 02-1-demo.mov    <- active working files
    └── ...
```
...
2. **Client-side:**
   ...
   - Simple pattern: socket event → `queryClient.invalidateQueries(['thumbs'])`
...
Small folder icon (📁) next to folder references throughout the app:

```
Config page:
┌─────────────────────────────────────────────────────────────────┐
│  Ecamm Watch Directory:  [~/Movies/Ecamm Live Recordings] [📁]  │
│  Target Directory:       [~/dev/video-projects/.../recordings] [📁]  │
│  Image Source Directory: [~/Downloads] [📁]                     │
└─────────────────────────────────────────────────────────────────┘

Incoming page header:
┌─────────────────────────────────────────────────────────────────┐
│  INCOMING FILES (~/Movies/Ecamm Live Recordings)  [📁]          │
└─────────────────────────────────────────────────────────────────┘

Recordings page header:
┌─────────────────────────────────────────────────────────────────┐
│  RECORDINGS (recordings/)  [📁]    SAFE (-safe/)  [📁]          │
└─────────────────────────────────────────────────────────────────┘

Assets page:
┌─────────────────────────────────────────────────────────────────┐
│  INCOMING IMAGES (~/Downloads)  [📁]                            │
│  ASSIGNED ASSETS (assets/images/)  [📁]                         │
└─────────────────────────────────────────────────────────────────┘

Thumbs page:
┌─────────────────────────────────────────────────────────────────┐
│  IMPORT FROM ZIP (~/Downloads)  [📁]                            │
...
├── b64-bmad-claude-sdk.mp4.txt  (transcript)
└── b64-bmad-claude-sdk.srt      (timing source of truth)
```
...
**2. Display in Projects panel (ⓘ popup)**
...
│  Last edit:  3 days ago │
│  ─────────────────────  │
│  Final Video: ✅ 4.4 GB │  ← NEW
│  Final SRT:   ✅        │  ← NEW
│  Location:   final/     │  ← NEW (or s3-staging/ or root)
│  ─────────────────────  │
│  Recordings: 12         │
│  Safe:       3          │
│  Transcripts:12/15 (80%)│
...
│  📌 │   b72-flivideo-recording-demo   │ EDIT  │ 12 │    28 │ 100%  │ ✅ │ ⓘ        │
│  📌 │   b74-multi-agent-workflows     │  REC  │  4 │     8 │   0%  │ ❌ │ ⓘ        │
│     │   b64-bmad-claude-sdk           │ DONE  │ 87 │    87 │ 100%  │ ✅ │ ⓘ        │
└────────────────────────────────────────────────────────────────────────────────────┘
```
...
function findLatestVersion(videos: string[]): { path: string; version?: number } {
  // Extract version numbers: b64-final-v3.mp4 → 3
...
```
┌─────────────────────────────────────────────────────────────────┐
│  CHAPTERS (b64-bmad-claude-sdk)                    [📋 Copy All] │
├─────────────────────────────────────────────────────────────────┤
│  ✅ 00:00   Intro                                               │
│  ✅ 02:34   Setting up BMAD                                     │
│  ⚠️ 05:12   First Implementation        [confidence: 72%]       │
│  ✅ 08:45   Testing                                             │
│  ❌ ???     Debugging                   [not found]             │
│  ✅ 12:30   Final Demo                                          │
└─────────────────────────────────────────────────────────────────┘
...
```
08 Voz Boy Baker.mov (1 file)      ← 08-1-voz-boy-baker.mov.mov (note: .mov in name!)
08 Voz Boy Baker (1 file)          ← 08-2-voz-boy-baker.mov

11 Architect Architecture (1 file)           ← 11-1-architect-architecture.mov
11 Architect Architecture Full Review (1 file) ← 11-2-architect-architecture-full-review.mov

20 Claude Sdk Configuration (1 file)              ← 20-1-claude-sdk-configuration.mov
20 Claude Sdk Configuration AUTHENTICATED (2 files) ← 20-2, 20-3 with -AUTHENTICATED tag
```
...
├── index.ts           # Router setup, composes sub-routers
