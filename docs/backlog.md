...
| ------ | --------------------------------------------------------------------------------------------- | ---------- | --------------------------------------------------------------- |
| FR-172 | [Read the Suite Word Store for the Whisper Prompt](prd/fr-172-suite-word-store.md)     | 2026-09-24 | Pending — LOW, for the rebuild (ticket only; fli-core v0.11.0 `readWords`) |
...
| ID      | Requirement                                                                       | Added      | Status                                                                                                       |
| ------- | --------------------------------------------------------------------------------- | ---------- | ------------------------------------------------------------------------------------------------------------ |
| NFR-172 | Gate hygiene: 20 pre-existing lint errors (`StoragePanel.tsx` rules-of-hooks ×7, `react-hooks/refs` in 4 client files, `no-unsafe-function-type` in 3 server tests, `miccheck-worklet.js`); `server/vitest.config.ts` has `coverage` outside `test` so thresholds never fire; stale gitignored `server/dist/` doubles vitest runs | 2026-09-15 | Pending (W3 review F10 — W3 bar taken as "no new lint problems, thresholds unchanged"; see docs/kdd/learnings.md 2026-09-15) |
