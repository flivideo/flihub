...
```typescript
async function deleteDerivableFiles(oldFilename: string, paths: ProjectPaths): Promise<void> {
  ...
  await Promise.all(
    [...shadowPaths, ...transcriptPaths].map(
      (p) => fs.unlink(p).catch(() => {}) // Swallow ENOENT errors
    )
  );
}
```
