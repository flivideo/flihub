...
4. Editor clicks **Collect** to pull files from relay into their local project `recordings/` folder.
...
### Managing project lifecycle (FR-148 redesign)
...
5. The **Copy Transcript** button in the Projects drawer (FR-148) calls `/api/query/projects/{code}/transcript/text` and copies the combined project transcript to clipboard with character-count feedback.
...
- **Pure filter functions extracted from components (FR-148)**: `client/src/utils/projectFilters.ts` contains `filterProjects()` as a standalone pure function separate from `ProjectsPanel`. *Alternative*: Inline logic inside the component. *Why*: The filter logic (needs-attention, dead, ready-to-edit presets) involves date calculations and multi-field comparisons that are easy to mis-test in a component context. Extraction allows the timestamp to be injected (`now` parameter), making preset filters unit-testable without mocking `Date.now()`.

- **Stage display config extracted to `constants/stages.ts` (FR-148)**: Stage label, badge colors, and descriptions are defined once in `client/src/constants/stages.ts` and imported by `ProjectListToolbar`, `ProjectDrawer`, and `ProjectsPanel`. *Alternative*: Inline the config in each component. *Why*: Three components needed the same config; duplication would cause drift (a label change in one place wouldn't update the others). Extraction is the minimal abstraction for a three-consumer problem.
...
- **Smart preset `dead` uses file-count, not content** — The "Dead" preset flags projects with ≤2 files AND 30+ days of inactivity. Two-file thresholds are arbitrary. A project with 2 recordings from 31 days ago that is deliberately paused will be flagged as dead. The preset is a signal, not a verdict.
...
- **The Projects drawer is a read-only diagnostic, not an edit surface** — The drawer's health assessment, checklist, and stats are computed from `ProjectStats` (a server snapshot). Quick actions (Open in Finder, Copy Transcript) go directly to server APIs. But stage changes and active project selection route through separate `useUpdateProjectStage` and config mutations. An expert reads the drawer for triage and acts through the dedicated controls, not by expecting the drawer itself to be an editor.
