...
import {
  ...
  useNextImageOrder,
  useAssignImage,
  useDeleteIncomingImage,
  useRefetchIncomingImages,
  useSavePrompt,
  useLoadPrompt,
  useDeletePrompt,
  useDeleteAssignedImage,
  useClipboardAssign,
  useClipboardToIncoming,
} from '../hooks/useAssetApi';
...
const THUMBNAIL_SIZES: Record<
  ThumbnailSize,
  { width: string; gridCols: string; textAreaHeight: string }
> = {
  S: {
    width: '120px',
    gridCols: 'grid-cols-4 sm:grid-cols-6 md:grid-cols-8',
    textAreaHeight: 'max-h-10',
  }, // ~40px, ~2 lines
  M: {
    width: '180px',
    gridCols: 'grid-cols-3 sm:grid-cols-4 md:grid-cols-5',
    textAreaHeight: 'max-h-28',
  }, // ~112px, ~6 lines
  L: {
    width: '25vw',
    gridCols: 'grid-cols-2 sm:grid-cols-3 md:grid-cols-4',
    textAreaHeight: 'max-h-52',
  }, // ~208px, ~12 lines
  XL: {
    width: '35vw',
    gridCols: 'grid-cols-1 sm:grid-cols-2 md:grid-cols-3',
    textAreaHeight: 'max-h-64',
  }, // ~256px, ~15 lines
};
...
export function AssetsPage() {
  ...
  const chapterMap = useMemo(() => {
    ...
    for (const [chapter, sequences] of map) {
      map.set(
        chapter,
        sequences.sort((a, b) => parseInt(a, 10) - parseInt(b, 10))
      );
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        if (assignment.variant) {
          const nextVariant =
            assignment.variant === 'a' ? 'b' : assignment.variant === 'b' ? 'c' : null;
          if (nextVariant) {
  ...
  const handleGrabTranscript = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_URL}/api/transcriptions/transcript/${encodeURIComponent(matching.filename)}`
      );
  ...
  const handlePopulateFromPairedAsset = (pair: PairedAsset) => {
    ...
    toast.success(
      `Controls set to ${pair.chapter}-${pair.sequence}${pair.variant ? `-${pair.variant}` : ''} "${pair.label}"`
    );
  };
...
interface ImageCardProps {
  ...
  shiftHeld: boolean;
  onPreviewEnter: (
    image: { url: string; filename: string; size: number; timestamp: string },
    e: React.MouseEvent
  ) => void;
