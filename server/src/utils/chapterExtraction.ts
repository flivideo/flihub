...
function findMatchInSrt(
  ...
  const startPositions =
    skipFirstWords > 0 ? [skipFirstWords, skipFirstWords + 5, skipFirstWords + 10] : [0, 1, 2];
...
export async function extractChapters(
  ...
  for (const info of chapterInfos) {
    ...
    if (match) {
      ...
      const alternatives = allCandidates.filter(
        (c) =>
          Math.abs(c.timestampSeconds - segment.startSeconds) > 5 &&
  ...
  const formatted = sortedForFormat.map((r) => `${r.timestamp} ${r.displayName}`).join('\n');

  const elapsedMs = Date.now() - startTime;
  console.log(
    `Chapter extraction completed in ${elapsedMs}ms (${segments.length} SRT segments, ${cleanResults.length} chapters)`
  );
