...
export async function appendTelemetryEntry(entry: TranscriptionLogEntry): Promise<void> {
  try {
    ...
    console.log(
      `Telemetry logged: ${entry.project}/${entry.filename} - ${entry.transcriptionDurationSec.toFixed(1)}s for ${entry.videoDurationSec.toFixed(1)}s video (${entry.ratio.toFixed(2)}x)`
    );
  } catch (err) {
